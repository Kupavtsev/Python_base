    ...
    def __attrnames(self):
        ...
        for attr in dir(self):                                      # Передать экземпляр функции dir()
            if attr[:2] == '__' and attr[-2:] == '__':              # Пропустить
                ...
                result += '\tname %s=%s\n' % (attr, getattr(self, attr))    # getattr показывает отрибуты из дерева наследования
        return result                                                       # которых нету в __dict__


# все дерево классов и составляет список атрибутов всех объектов, находящихся в дереве выше
class ListTree:
    def __str__(self):
        self.__visited = {}
